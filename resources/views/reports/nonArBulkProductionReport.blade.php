        ...
        <div class="card-body p-0">
            <form id="filterForm">
                <div class="card-header border-0 px-4" style="background-color: #139AB3;height: 84px">
                    <div class="row">
                        <div class="col-md-6">
                            <span class="project_header" style="margin-left: 4px !important;color: #ffffff;">Non Ar Projects Report</span>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#filter_clear').on('click', function(e) {
                window.location.href = baseUrl + 'reports/non_ar_report'
                "?parent=" +
                getUrlVars()[
                        "parent"] +
            ...
            $('#formUpdate_save').on('click', function(e) {
                ...
                $.post("{{ url('non-ar-run-python') }}", formData, function(res) {
            ...
            function checkFileReady(jobId) {
                var interval = setInterval(function() {
                    $.get("{{ url('non-ar-check-report') }}/" + jobId, function(res) {
                        if (res.ready) {
                            clearInterval(interval);
                            window.location.href = "{{ url('non-ar-download-report') }}/" + res.file;
                            KTApp.unblock('#filterForm');
                        }
                    }).fail(function() {
                        clearInterval(interval);
