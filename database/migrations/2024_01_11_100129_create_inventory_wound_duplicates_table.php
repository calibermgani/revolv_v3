...
class CreateInventoryWoundDuplicatesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('inventory_wound_duplicates', function (Blueprint $table) {
            $table->id();
            ...
            $table->enum('status',['CE_Inprocess','CE_Pending','CE_Completed','CE_Hold','QA_Inprocess','QA_Pending','QA_Completed','QA_Hold','agree','dis_agree'])->default('CE_Inprocess');
            $table->timestamps();
            $table->softDeletes();
        });
    }
    ...
    {
        Schema::dropIfExists('inventory_wound_duplicates');
    }
}
