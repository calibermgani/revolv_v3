            ...
            <div class="card card-custom custom-top-border">
                            ...
                            @foreach ($projectColSearchFields as $key => $data)
                             ...
                             $subProjectName = $data->sub_project_id != null ? App\Http\Helper\Admin\Helpers::encodeAndDecodeID($data->sub_project_id, 'encode') : '--';
                                @endphp
                                 {!! Form::open([
                                    'url' =>
                                    url('qa_production/qa_rebuttal/' . $clientName . '/' . $subProjectName) .
                                                    '?parent=' .
                                                    request()->parent .
                                                    '&child=' .
                                                    request()->child,
                ...
                <div class="card-body py-0 px-7">
                                            ...
                                            @foreach ($arrayAttrributes as $columnName => $columnValue)
                                                @php
                                                    $columnsToExclude = [
                                                        ...
                                                        'CE_sub_status_code',
                                                        'CE_followup_date',
                                                       'cpt_trends','icd_trends','modifiers','annex_coder_trends','annex_qa_trends',
                                                        'qa_cpt_trends',
                                                        'qa_icd_trends',
                                                        'qa_modifiers',
                                                        'created_at',
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="ml-3">
                            Showing {{ $rebuttalProjectDetails->firstItem() != null ? $rebuttalProjectDetails->firstItem() : 0 }} to {{ $rebuttalProjectDetails->lastItem() != null ? $rebuttalProjectDetails->lastItem() : 0 }} of {{ $rebuttalProjectDetails->total() }} entries
                        </div>
                         <div>
                            {{ $rebuttalProjectDetails->appends(request()->all())->links() }}
                        </div>
                    </div>
                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-9" style="border-left: 1px solid #ccc;" data-scroll="true"
                                    ...
                                    @if (count($popupEditableFields) > 0)
                                        ...
                                        <hr>
                                        <h6 class="title-h6">Rebuttal Action</h6>
                                        <div class="row mt-3">
                                            <div class="col-md-6">
                                                <div class="form-group row">
                                                    <label class="col-md-12" id="ar_manager_rebuttal_status_label">
                                                        AR Status
                                                    </label>
                                                    <label class="col-md-12 pop-non-edt-val" id="ar_manager_rebuttal_status">
                                                    </label>
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group row">
                                                    <label class="col-md-12" id="ar_manager_rebuttal_comments_label">
                                                       AR Comments
                                                    </label>
                                                    <label class="col-md-12 pop-non-edt-val" id="ar_manager_rebuttal_comments">
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row mt-4">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group row">
                                                    <label class="col-md-12 required" id="qa_manager_rebuttal_comments_label" style = 'display:none'>
                                                        Comments
                                                    </label>
                                                    <div class="col-md-10">
                                                        {!! Form::textarea('qa_manager_rebuttal_comments', null, [
                                                            'class' => 'text-black form-control',
                                                            'rows' => 3,
                                                            'id' => 'qa_manager_rebuttal_comments',
                                                            'style' => 'display:none',
                                                            'required',
                                                        ]) !!}
        ...
        $(document).ready(function() {
            ...
            var table = $("#client_rebuttal_list").DataTable({
                ...
                buttons: [{
                    ...
                    "title": 'Resolv',
                    "filename": 'resolv_rebuttal_' + date,
                    "exportOptions": {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success == true) {
                            ...
                            headers.push('qa_manager_rebuttal_status');
                            headers.push('qa_manager_rebuttal_comments');
                    ...
                    $.each(headers, function(index, header) {
                        ...
                        if (/_el_/.test(value)) {
                            ...
                            if (header == 'qa_manager_rebuttal_status') {
                                $('select[name="qa_manager_rebuttal_status"]').val(value);
                            }
                            if (header == 'qa_manager_rebuttal_comments') {
                                $('textarea[name="qa_manager_rebuttal_comments"]').val(value);
                                if (value !== null) {
                                    $('#qa_manager_rebuttal_comments_label').css('display',
                                    'block');
                                    $('#rework_reason').css('display', 'block');
                                } else {
                                    $('#qa_manager_rebuttal_comments_label').css('display', 'none');
                            ...
                            if (header == 'ar_manager_rebuttal_status') {
                                let capitalizedWord = capitalizeFirstLetter(value);
                                 $('label[id="ar_manager_rebuttal_status"]').text(capitalizedWord);
                            }
            ...
            $(document).on('click', '#project_rebuttal_save', function(e) {
                e.preventDefault();
                var qa_manager_rebuttal_status = $('#qa_manager_rebuttal_status').val();
                var qa_manager_rebuttal_comments = $('#qa_manager_rebuttal_comments').val();
                var parentIdVal = $('input[name="parentId"]').val();
                var chargeStatus = $('#chart_status').text();
                if (qa_manager_rebuttal_status == '') {
                    $('#qa_manager_rebuttal_status').css('border-color', 'red');
                    return false;
                } else {
                    $('#qa_manager_rebuttal_status').css('border-color', '');
                }
                if (qa_manager_rebuttal_comments == '') {
                    $('#qa_manager_rebuttal_comments').css('border-color', 'red');
                    return false;
                } else {
                    $('#qa_manager_rebuttal_comments').css('border-color', '');
                }
                swal.fire({
                    ...
                    if (result.value == true) {
                        ...
                        $.ajax({
                            type: "GET",
                            url: "{{ url('qa_production/qa_rebuttal_update') }}",
                            data: {
                                clientName: clientName,
                                subProjectName: subProjectName,
                                qa_manager_rebuttal_status: qa_manager_rebuttal_status,
                                qa_manager_rebuttal_comments: qa_manager_rebuttal_comments,
            ...
            $(document).on('click', '#filter_clear', function(e) {
                window.location.href = baseUrl + 'qa_production/qa_rebuttal/' + clientName + '/' + subProjectName +
                    "?parent=" +
                    getUrlVars()[
                        "parent"] +
                    "&child=" + getUrlVars()["child"];
            })
            $(document).on('change', '#qa_manager_rebuttal_status', function() {
                var qa_manager_rebuttal_status = $(this).val();console.log(qa_manager_rebuttal_status,'qa_manager_rebuttal_status');
                
                if (qa_manager_rebuttal_status) {
                    $('#qa_manager_rebuttal_comments').css('display', 'block');
                    $('#qa_manager_rebuttal_comments_label').css('display', 'block');
                } else {
                    $('#qa_manager_rebuttal_comments').css('display', 'none');
                    $('#qa_manager_rebuttal_comments_label').css('display', 'none');
                    $('#qa_manager_rebuttal_comments').css('border-color', '');
