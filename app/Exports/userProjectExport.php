        ...
        foreach ($projectDetails as $project) {
          ...
          if($subProjectName !== '--' && !empty($matchKey) && in_array($project['sub_prj_id'], $this->subPrjIds[$matchKey[0]]))  {  
                ...
                foreach ($this->periods as $date) {
                    ...
                    $decodedSubProjectName = $project['sub_prj_id'] == null
                        ? 'project'
                        : ($project['prj_id'] != null
                            ? (\App\Http\Helper\Admin\Helpers::subProjectName($project['prj_id'], $project['sub_prj_id'])['sub_project_name'] ?? null)
                            : null);
