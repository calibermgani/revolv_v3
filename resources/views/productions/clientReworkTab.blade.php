            ...
            <div class="card card-custom custom-top-border">
                            ...
                            @foreach ($projectColSearchFields as $key => $data)
                             ...
                             $subProjectName = $data->sub_project_id != null ? App\Http\Helper\Admin\Helpers::encodeAndDecodeID($data->sub_project_id, 'encode') : '--';
                                @endphp
                                 {!! Form::open([
                                    'url' =>
                                    url('projects_Revoke/' . $clientName . '/' . $subProjectName) .
                                                    '?parent=' .
                                                    request()->parent .
                                                    '&child=' .
                                                    request()->child,
                ...
                <div class="card-body py-0 px-7">
                                            ...
                                            <td>
                                                @if (
                                                    ($empDesignation !== 'Administrator' ||
                                                        ...
                                                        strpos($empDesignation, 'Vice') !== true || strpos($empDesignation, 'Group Coordinator') !== true || strpos($empDesignation, 'Subject Matter Expert') !== true) &&
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="ml-3">
                            Showing {{ $revokeProjectDetails->firstItem() != null ? $revokeProjectDetails->firstItem() : 0 }} to {{ $revokeProjectDetails->lastItem() != null ? $revokeProjectDetails->lastItem() : 0 }} of {{ $revokeProjectDetails->total() }} entries
                        </div>
                         <div>
                            {{ $revokeProjectDetails->appends(request()->all())->links() }}
                        </div>
                    </div>     
                </div>
                    ...
                    function handleClientData(clientData,headers) {
                        $.each(headers, function(index, header) {
                            ...
                            if (header == 'qa_scope') {
                                ...
                                if (header == 'QA_rework_comments') {
                                    console.log(value,'qa_rework_comments_label');
                                    
                                    if(value !== '') {
                                        $('#hr_view').css('display','block');
                                       $('label[id="qa_rework_comments_label"]').css('display','block');
                                    } else {
                                        $('#hr_view').css('display','none');
                                       $('label[id="qa_rework_comments_label"]').css('display','none');
                                    }
