    ...
    <div class="card card-custom custom-card" id="page-loader">
                ...
                <div class="card-toolbar d-inline float-right mt-3">
                    <div class="outside" href="javascript:void(0);" id="export_data">
                        <span data-dismiss="modal" class="btn btn-secondary--icon p-2 float-right  mb-3"
                            id="export_data" data-toggle="tooltip" data-placement="left" data-original-title="Export" style="background-color: #139AB3;font-size:13px;color: white">   <svg xmlns="http://www.w3.org/2000/svg" width="14" height="12" fill="currentColor" class="bi bi-box-arrow-up" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M3.5 6a.5.5 0 0 0-.5.5v8a.5.5 0 0 0 .5.5h9a.5.5 0 0 0 .5-.5v-8a.5.5 0 0 0-.5-.5h-2a.5.5 0 0 1 0-1h2A1.5 1.5 0 0 1 14 6.5v8a1.5 1.5 0 0 1-1.5 1.5h-9A1.5 1.5 0 0 1 2 14.5v-8A1.5 1.5 0 0 1 3.5 5h2a.5.5 0 0 1 0 1z"/><path fill-rule="evenodd" d="M7.646.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1-.708.708L8.5 1.707V10.5a.5.5 0 0 1-1 0V1.707L5.354 3.854a.5.5 0 1 1-.708-.708z"/>
        ...
        <div class="card-body pt-0 pb-2 pl-8" style="background-color: #ffffff !important">
            ...
            <div class="table-responsive pb-4">
                ...
                <table class="table table-separate table-head-custom no-footer dtr-column" id="project_report">
                    <thead>
                        <tr>
                        ...
                        @if (isset($prjDetailsList) && is_array($prjDetailsList))                           
                            @foreach ($prjDetailsList as $projectDetails)
                                @foreach ($projectDetails as $project)
                                    @php 
                                    $subProjectDetails = $project['prj_id'] != null && $project['sub_prj_id'] != null 
                                        ? App\Http\Helper\Admin\Helpers::subProjectName($project['prj_id'], $project['sub_prj_id']) 
                                        : null;

                                    $subProjectName = $subProjectDetails['sub_project_name'] ?? '--';                         
               ...
               // $('.daterange').attr("autocomplete", "off");
                ...
                $(document).on('change', '#manager_name', function() {
                    ...
                    $(document).on('click', '#export_data', function () {
                        ...
                        // force HTTPS
                        var exportUrl = "{{ secure_url('report/user_project_report_export') }}?" + $.param(query);

                        // trigger download
                        window.location.href = exportUrl;

                        // hide loader after 5s (download prompt already triggered)
                        setTimeout(() => {
                ...
                $(document).on('click', '#clear_submit', function(e) {
                    ...
                    work_date = 0;
                    $('.daterange').val('');
                    $('#project_report').DataTable().destroy();
